...
public class PluginMessageUtils {
    public static void getRank(Player player, Callback<Ranks> callback) {
        TomeitoAPI.getPluginChannelListener().get(player, player.getUniqueId().toString(), "", "dtc:rank", new Callback<String>() { // message isn't used
            @Override
    ...
    public static void get(Player player, String message, String s, Callback<String> callback) {
        TomeitoAPI.getPluginChannelListener().get(player, player.getUniqueId().toString(), message, s, new Callback<String>() { // message isn't used
            @Override
    ...
    public static void get(Player player, String message, String subchannel, String s, Callback<String> callback) {
        TomeitoAPI.getPluginChannelListener().get(player, subchannel, message, s, new Callback<String>() { // message isn't used
            @Override
            public void done(String s, Throwable e) {
                if (s == null && e != null) { // it shouldn't happen... if happened, check the proxy log
                    NullPointerException npe = new NullPointerException();
